...
def arp_monitor_page():
    """ARP Monitor management page"""
    return render_template('arp-monitor.html')

@app.route('/api/arp/status', methods=['GET'])
def api_arp_status():
    """Get ARP monitor status"""
    try:
        status = {
            'enabled': False,
            'learned_mac': None,
        ...
        result = subprocess.run(
            ['systemctl', 'is-active', 'arp-monitor.service'],
            capture_output=True,
            text=True
        )
        status['enabled'] = result.stdout.strip() == 'active'
        status['monitoring'] = status['enabled']
        
        # Get learned baseline
        try:
            with open('/run/arp-monitor/gateway-mac', 'r') as f:
                status['learned_mac'] = f.read().strip()
        except:
            pass
        ...
        if gateway_result.returncode == 0:
            ...
            if len(parts) >= 3:
                status['current_gateway_ip'] = parts[2]
                
                # Get MAC from ARP table
                arp_result = subprocess.run(
                    ['ip', 'neigh', 'show', status['current_gateway_ip'], 'dev', 'wlan0'],
                    capture_output=True,
                    text=True
                )
                ...
                if mac_match:
                    status['current_gateway_mac'] = mac_match.group(0)
...
@app.route('/api/arp/reset', methods=['POST'])
def api_arp_reset():
    """Reset learned gateway MAC"""
    try:
        subprocess.run(
            ['sudo', 'rm', '-f', '/run/arp-monitor/gateway-mac'],
            capture_output=True,
            timeout=5
        )
        
        subprocess.run(
            ['sudo', 'systemctl', 'restart', 'arp-monitor.service'],
            capture_output=True,
            timeout=10
        )
        ...
        return jsonify({
            'success': True,
            'message': 'ARP baseline reset. Monitor will relearn gateway MAC.'
        })
        
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/arp/enable', methods=['POST'])
def api_arp_enable():
    ...
    try:
        ...
        if result.returncode == 0:
            return jsonify({'success': True, 'message': 'ARP monitoring enabled'})
        else:
...
@app.route('/api/arp/disable', methods=['POST'])
def api_arp_disable():
    """Disable ARP monitoring"""
    try:
        result = subprocess.run(
            ['sudo', 'systemctl', 'disable', '--now', 'arp-monitor.service'],
            capture_output=True,
            timeout=10
        )
        
        if result.returncode == 0:
            return jsonify({'success': True, 'message': 'ARP monitoring disabled'})
        else:
            return jsonify({'success': False, 'error': result.stderr}), 500
